...
#DETECTAR SI ES MEDICO o ANALISTA
def is_analist(user):
    return user.groups.filter(name='analista').exists()

@user_passes_test(is_analist)
def redirect_repo(request):
    repo = "https://github.com/JosiasRL/MLAnemia"
    return redirect(repo)

def some_view(request):
    context = {
        'is_analista': is_analist(request.user),
    }
